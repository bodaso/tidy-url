    ...
    TidyCleaner.prototype.clean = function (_url, allowReclean) {
        ...
        if (this.config.allowRedirects) {
            for (var _k = 0, _l = data.info.match; _k < _l.length; _k++) {
                ...
                var isEncoded = (0, utils_1.decodeURL)(value, interface_1.EEncoding.urlc);
                if (isEncoded !== value && (0, utils_1.validateURL)(isEncoded))
                    value = isEncoded;
                if (target.length && cleaner_ci.has(target)) {
                    if ((0, utils_1.validateURL)(value)) {
                        data.url = "".concat(value) + original.hash;
                        if (allowReclean)
        ...
        for (var _p = 0, _q = data.info.match; _p < _q.length; _p++) {
            var rule = _q[_p];
            try {
                this.log("Processing decode rule (".concat(rule.name, ")"), 'debug');
                if (!rule.decode)
                    continue;
                ...
                if ((0, utils_1.isJSON)(decoded)) {
                    ...
                    if (rule.decode.handler && handler) {
                        ...
                        if (result.error || (0, utils_1.validateURL)(result.url) === false || result.url.trim() === '') {
                            if (result.error)
        ...
        if (data.info.reduction < 0) {
            this.log("Reduction is ".concat(data.info.reduction, ". Please report this link on GitHub: ").concat($github, "/issues\n").concat(data.info.original), 'error');
            data.url = data.info.original;
        }
